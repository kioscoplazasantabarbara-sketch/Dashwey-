...
const CACHE_NAME  = 'dashwey-v1-1-6-dev';
    ...
    caches.keys()
      .then(keys => {
        ...
        return Promise.all(toDelete.map(k => {
          console.log('[SW v1.1.6-dev] Install: eliminando cache antigua:', k);
  ...
  e.waitUntil(
    caches.keys()
      .then(keys => {
        const toDelete = keys.filter(k => k !== CACHE_NAME);
        return Promise.all(toDelete.map(k => {
          console.log('[SW v1.1.6-dev] Activate: eliminando cache antigua:', k);
          return caches.delete(k);
        }));
      })
      ...
      .then(clients => {
        if (clients.length > 0) {
          ...
          if (permission === 'granted') {
            self.registration.showNotification('🆕 Dashwey v1.1.6-dev disponible', {
              ...
              renotify: true,
              vibrate:  [100, 50, 100],
              data:     { action: 'update', version: '1.1.6-dev', url: '/' },
            }).catch(() => {});
          }
        }
      })
