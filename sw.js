/* Dashwey Service Worker v9.3.0 */
const CACHE_NAME  = 'dashwey-v9-3-0';
const HTML_URL    = 'Dashwey_v82.html';
const VERSION_URL = 'version.txt';
self.addEventListener('install', e => {
  e.waitUntil(caches.open(CACHE_NAME).then(c=>c.addAll([HTML_URL,VERSION_URL]).catch(()=>{})).then(()=>self.skipWaiting()));
});
...
self.addEventListener('fetch', e => {
  ...
  if(url.pathname.endsWith(HTML_URL)||url.pathname.endsWith('/')){
    e.respondWith(fetch(e.request,{cache:'no-store'}).then(res=>{
      if(res?.status===200){const c=res.clone();caches.open(CACHE_NAME).then(ca=>ca.put(e.request,c));}
      return res;
  ...
  e.respondWith(caches.match(e.request).then(cached=>{
    ...
    return fetch(e.request).then(res=>{
      if(res?.status===200&&res.type!=='opaque'){const c=res.clone();caches.open(CACHE_NAME).then(ca=>ca.put(e.request,c));}
      return res;
    });
  }));
});
