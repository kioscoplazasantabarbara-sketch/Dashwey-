/* ═══════════════════════════════════════════════════════════════════
   Dashwey Service Worker v9.5.92
...
const CACHE_NAME   = 'dashwey-v9-5-92';
...
/* ── Install ────────────────────────────────────────────────────────
   v9.5.92: skipWaiting inmediato + limpieza agresiva de caches viejas
    ...
    caches.keys()
      .then(keys => {
        ...
        return Promise.all(toDelete.map(k => {
          console.log('[SW v9.5.92] Install: eliminando cache antigua:', k);
...
/* ── Activate ───────────────────────────────────────────────────────
   v9.5.92: claim + reload de TODOS los clientes.
  ...
  e.waitUntil(
    caches.keys()
      .then(keys => {
        const toDelete = keys.filter(k => k !== CACHE_NAME);
        return Promise.all(toDelete.map(k => {
          console.log('[SW v9.5.92] Activate: eliminando cache antigua:', k);
          return caches.delete(k);
        }));
      })
