/* ═══════════════════════════════════════════════════════════════════
   Dashwey Service Worker v9.5.61
...
const CACHE_NAME   = 'dashwey-v9-5-61';
...
/* ── Install ────────────────────────────────────────────────────────
   v9.5.61: skipWaiting inmediato + limpieza agresiva de caches viejas
    ...
    caches.keys()
      .then(keys => {
        ...
        return Promise.all(toDelete.map(k => {
          console.log('[SW v9.5.61] Install: eliminando cache antigua:', k);
...
/* ── Activate ───────────────────────────────────────────────────────
   v9.5.61: claim + reload de TODOS los clientes.
   El reload fuerza que los clientes descarguen el HTML fresco de la red
   (nunca del cache, ya que el HTML no se cachea en v9.5.61).
  ...
  e.waitUntil(
    caches.keys()
      .then(keys => {
        const toDelete = keys.filter(k => k !== CACHE_NAME);
        return Promise.all(toDelete.map(k => {
          console.log('[SW v9.5.61] Activate: eliminando cache antigua:', k);
          return caches.delete(k);
        }));
      })
...
/* ── Fetch ──────────────────────────────────────────────────────────
   v9.5.61 ESTRATEGIA DE CACHE:

   HTML principal → NETWORK-ONLY (nunca se cachea)
   ─────────────────────────────────────────────────────────────────
   CAMBIO CRÍTICO v9.5.61: el HTML YA NO SE GUARDA en cache.
...
self.addEventListener('fetch', e => {
  ...
  /* HTML principal: NETWORK-ONLY — NUNCA se cachea en v9.5.61.
