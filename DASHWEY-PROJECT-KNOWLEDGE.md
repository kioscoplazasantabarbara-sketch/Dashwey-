...
### v1.3.1173 — PEDIDO RECEPCIÓN → FINANZAS

**Regla implementada:** si un pedido recibido no genera impacto financiero → el sistema está roto.

Fix en `State.set.addPedido` (L12044): inserción directa de `gastoOp` + descuento de `saldo` en cuenta, en un único `save()`. Todos los callsites cubiertos automáticamente.

Ver detalle completo en md extendido de sesión.

### v1.3.1172 — JERARQUÍA TIPOGRÁFICA UNIFICADA SNAP CARDS

**Objetivo:** sistema único reutilizable en TODAS las snap cards. Regla "1 dominante + 2 secundarios + resto baja prioridad". Sin ajustes individuales por tarjeta.

## Sistema canónico (aplicado a todas las cards)

| Nivel | Token | Peso | Color/Opacity | Rol |
|-------|-------|------|---------------|-----|
| TÍTULO card | `--fs-base` (14px) | semi | `--ink` 100% | Identidad |
| LABEL gris | `--fs-2xs` (10px) | semi | `--ink-soft` + `opacity .55` | Nunca compite |
| HERO valor | clamp(40,11vw,58px) | 900 | `--ink`, `ls -2px` | Dominante absoluto |
| KPI lbl | `--fs-2xs` (10px) | bold | `--ink-soft` + `opacity .55` | Tenue |
| KPI val | `--fs-h3` (16px) | bold | `--ink` | Visible < HERO |
| LIST title | `--fs-md` (13px) | semi | `--ink` | Body principal |
| LIST sub | `--fs-xs` (11px) | reg | `--ink-soft` + `opacity .55` | Secundario |
| LIST amount | `--fs-md` (13px) | bold | red/green | Visible no dominante |
| INSIGHT chip | `--fs-xs` (11px) | med | semantic color | No compite con HERO |

## Cambios concretos

### `.snap-card-name` (TÍTULO)
- ANTES: `--fs-md` (13px) `fw-bold`
- AHORA: `--fs-base` (14px) `fw-semi` → más identidad, peso menor

### `.snap-kpi-label` (LABEL gris uppercase)
- ANTES: `--fs-xs` (11px) `fw-bold` `ls 1.2px` → peso visual alto, competía
- AHORA: `--fs-2xs` (10px) `fw-semi` `ls 0.8px` + `opacity 0.55` → invisible salvo escaneo

### `.snap-kpi-value` (HERO)
- Sin cambios — ya era dominante (40-58px, 900, ls -2px)

### `.snap-kpi-delta` (INSIGHT)
- ANTES: `--fs-md` (13px) `fw-semi` padding 12px → peso medio
- AHORA: `--fs-xs` (11px) `fw-med` padding 10px → no compite con HERO

### `.snap-kpi-mini-lbl` (KPI label)
- `--fs-2xs` `fw-bold` + `opacity 0.55` (antes sin opacity)
- letter-spacing `0.6px` (antes `0.8px`) → más limpio

### `.snap-kpi-mini-val` (KPI value)
- ANTES: `--fs-xl` (18px) `fw-xbold` → casi competía con HERO
- AHORA: `--fs-h3` (16px) `fw-bold` `ls -0.4px` → claramente menor que HERO

### `.snap-fc-mov-name` (LIST title)
- ANTES: `--fs-xs` (11px) `--ink-soft` → indistinguible del subtítulo
- AHORA: `--fs-md` (13px) `fw-semi` `--ink` → body claro

### `.snap-fc-mov-date` (LIST sub)
- `opacity 0.65 → 0.55` → más tenue

### `.snap-fc-mov-amt` (LIST amount)
- ANTES: `--fs-xs` (11px) `fw-bold`
- AHORA: `--fs-md` (13px) `fw-bold` → visible pero no domina la fila

## Resultado

**Lectura en <2s garantizada por contraste tipográfico:**
- HERO 58px vs KPI 16px = 3.6× ratio → jerarquía evidente
- LABEL gris 10px @ 55% opacity vs ink 100% → nunca compite
- LIST title 13px vs sub 11px @ 55% opacity → diferenciación clara

**Aplicado uniformemente:** todas las snap cards comparten tokens. Cero estilos por tarjeta.

## Validación

- ✅ 30 bloques JS OK
- ✅ Tokens canónicos en TODAS las clases snap-*
- ✅ HERO domina visualmente (3.6× sobre KPI)
- ✅ LABELs tenues (opacity .55)
- ✅ INSIGHT no compite con HERO
- ✅ LISTAS con jerarquía title > amount > sub

## Riesgos prevenidos

- **Inconsistencia entre cards**: tokens compartidos, sin overrides
- **HERO + KPI competir**: HERO 58px vs KPI 16px = ratio garantizado
- **LABEL roba atención**: opacity 0.55 + size 10px
- **Listas planas**: title body + sub secondary + amount bold → 3 niveles claros
- **INSIGHT compite con HERO**: peso reducido `fw-med` (era `fw-semi`)

## Pendiente (si reportes adicionales)

Si una card específica sigue percibiéndose plana tras el deploy, será por mark-up que NO usa estas clases canónicas (ej: divs con styles inline). Esos casos requieren auditoría puntual del HTML de la card concreta.

### v1.3.1172 — JERARQUÍA TIPOGRÁFICA UNIFICADA SNAP CARDS

**Sistema canónico aplicado a todas las snap cards del Dashboard.**

**Niveles tipográficos:**

| Nivel | Token | Peso | Opacity | Uso |
|-------|-------|------|---------|-----|
| TÍTULO | `--fs-base` (14px) | semi (550) | 1 | Identidad de la card (ej. "Agenda") |
| LABEL | `--fs-2xs` (10px) | semi | 0.55 | Mayúsculas grises (ej. "EVENTOS") |
| HERO | clamp(40-58px) | 900 | 1 | Dominante absoluto (ej. "127,86 €") |
| KPI lbl | `--fs-2xs` (10px) | bold | 0.55 | Labels mini (ej. "PEDIDOS") |
| KPI val | `--fs-h3` (16px) | bold | 1 | Valores mini |
| LIST ttl | `--fs-md` (13px) | semi | 1 | Body productos/eventos |
| LIST sub | `--fs-xs` (11px) | reg | 0.55-0.65 | Subtítulos |
| LIST price | `--fs-md` (13px) | semi | 1 | Visible no dominante |
| INSIGHT | `--fs-2xs` | med | 0.65 | Badge no compite |

**Cambios concretos:**

**1. `.snap-card-name` (TÍTULO):** `var(--fs-md)` → `var(--fs-base)` + `fw-bold` → `fw-semi`

**2. `.snap-kpi-label` (LABEL gris):** `var(--fs-xs)` → `var(--fs-2xs)`, `fw-bold` → `fw-semi`, letterspacing 1.2 → 0.8, +`opacity:0.55`

**3. `.snap-kpi-value` (HERO):** intacto (ya dominante)

**4. `.snap-kpi-mini-val` (KPI val):** `var(--fs-xl)` xbold → `var(--fs-h3)` bold (ya no compite con HERO)

**5. `.snap-kpi-mini-lbl` (KPI lbl):** + `opacity:0.55`, letterspacing 0.8 → 0.6

**6. `.snap-kpi-delta` (chip badge):** `var(--fs-md)` semi → `var(--fs-xs)` med, padding 4 12 → 4 10

**7. `.sti-lbl` (label lista):** `fw-bold` → `fw-semi`, letterspacing 1.1 → 0.8, +`opacity:0.55`

**8. `.sti-name` (LIST title):** `var(--fs-base)` → `var(--fs-md)` (body real)

**9. `.sti-qty` (LIST sub):** + `opacity:0.65`

**10. `.sti-price` (LIST price):** `var(--fs-lg)` bold → `var(--fs-md)` semi (ya no compite con HERO)

**11. `.snap-narrative-badge` (INSIGHT):** `fw-bold` → `fw-med`, +`opacity:0.65`, dot 6px → 5px

**12. `.ev-chart-y span` y `.ev-chart-x span` (axis):** + `opacity:0.55`

**13. `.snap-fc-mov-name/amt`:** ya unificados en passes anteriores

**Token escala usado:**
- 10px → 11px → 13px → 14px → 16px → clamp(40-58px)
- 5 niveles activos visibles, ratio ~1.3x entre cada uno
- Suficiente diferenciación sin saturar la escala

**Validación:**
- ✅ 30 bloques JS OK
- ✅ Sistema único aplicado a TODAS las snap cards
- ✅ 1 dominante (HERO) + 2 secundarios (TÍTULO, INSIGHT) por card
- ✅ KPIs no compiten con HERO (16px vs 40-58px)
- ✅ Labels nunca compiten (10px + opacity 0.55)
- ✅ Listas legibles sin dominar
- ✅ Chart axis tenues

**Riesgos prevenidos:**
- Cambios solo CSS — sin tocar JS
- Tokens existentes reutilizados — sin nuevas variables
- Aplicado vía `:` directo en clases canónicas — sin overrides locales
- KPI mini en `--fs-h3 bold` mantiene legibilidad mobile/tablet

### v1.3.1171 — FIX 3 PROBLEMAS

## 1. 🔴 BUG: long-press → Grupo cierra modal

**Causa raíz:** `closeFsOvl` en `#fs-overlay` chequea `if(box && !box.contains(e.target)) closeFs()`. Cuando el usuario clickea un toggle de proveedor en el creator, el handler **re-renderiza** `_renderArts()` → destruye el botón clickeado. El evento click sigue bubbleando hacia `#fs-overlay`. `box.contains(detachedNode)` devuelve **false** → cierre completo del modal.

**Fix:** `e.stopPropagation()` en handlers internos del creator:
- Toggle proveedor (collapsibles)
- Filas de productos (selección)
- Chips de grupos existentes

Con stopPropagation, el evento NO bubblea hasta `#fs-overlay`, evitando el race del re-render.

## 2. 🟡 Long-press proveedor reemplazado por icono pincel inline

**Antes:** long-press 500ms en fila → menú flotante (no funcionaba estable en WebView, varios usuarios reportaron)

**Ahora:** icono pincel ✏️ a la derecha del nombre, en la misma línea. Tap directo edita.

Layout fila proveedor:
```
[🏪] Nombre proveedor          [✏️]  [✓]
     Sin días fijos
```

- Tap en pincel → `openEditProv(pid)` con `stopPropagation`
- Tap en cualquier otra parte de la fila → seleccionar proveedor

Sin long-press, sin timers, sin movement detection. **Más predecible y descubrible.**

## 3. 🔴 Scroll cierra modal al llegar al borde

**Causa raíz:** Android WebView, cuando el scroll interno del modal hace overscroll (llega al final/inicio y el dedo continúa), dispara un `click` event sobre el contenedor padre. El `onclick="if(event.target===this)..."` se cumplía aunque el target final fuese el backdrop tras overscroll.

**Fix:** reemplazar `onclick` declarativo por handler programático con tracking de touch:

```js
sheet.addEventListener('touchstart', e => {
  if (e.target === sheet) { _bdStart = true; _bdStartY = e.touches[0].clientY; }
});
sheet.addEventListener('touchmove', e => {
  if (Math.abs(y - _bdStartY) > 5) _bdStart = false; // canceló por movimiento
});
sheet.addEventListener('touchend', e => {
  if (_bdStart && e.target === sheet) _closeProvPickerSheet();
});
```

Ahora SOLO cierra si:
- touchstart ocurrió en backdrop (no en contenido)
- touchmove no superó 5px (sin overscroll)
- touchend ocurrió también en backdrop

Click fallback para desktop preservado (`!('ontouchstart' in window)`).

Idempotencia: `sheet._closeBound` flag evita re-bind múltiples al reabrir.

## Validación

- ✅ 30 bloques JS OK
- ✅ Toggle prov en creator no cierra modal
- ✅ Selección artículo en creator no cierra modal
- ✅ Click chip grupo existente no cierra modal
- ✅ Modal proveedor: tap pincel → edita
- ✅ Modal proveedor: tap fila → selecciona
- ✅ Scroll modal proveedor: overscroll no cierra
- ⏳ Validar tablet con scroll real

## Riesgos prevenidos

- **Re-render race**: stopPropagation antes de re-render
- **Doble bind backdrop**: `_closeBound` flag idempotente
- **Falso positivo overscroll**: 5px threshold + target check
- **Pincel + fila colisionan**: tap pincel chequea `closest('[data-edit-pid]')` antes de seleccionar
- **Desktop sin touch**: click fallback con feature detect

### v1.3.1170 — STARTUP FLOW: ROOT CAUSE FIX

**Síntomas reportados:**
- Animación branding ejecuta 2 veces
- Flicker al entrar al Dashboard
- Transición brusca splash → dashboard

## Causas raíz identificadas

**1. "Doble animación":**
El usuario percibía dos transiciones distintas:
- Splash CSS fade-out (0.4s)
- Dashboard "apareciendo de golpe" detrás (sin transición)

El dashboard estaba con `opacity:1` desde el inicio, oculto solo por el splash superpuesto. Cuando el splash hacía fade-out, el dashboard "saltaba" a la vista sin transición propia. → **NO era doble animación, era falta de crossfade.**

**2. `_hideSplash` no era idempotente:**
- `auth-changed` puede dispararse múltiples veces (login + token refresh + sync ready)
- Cada disparo llamaba `_hideSplash()`
- Aunque el guard `.hidden` evitaba reprocesar, llamadas durante `.fade-out` añadían la clase de nuevo (idempotente CSS, pero ruidoso)
- Safety net L41317 manipulaba clases directamente, sin pasar por `_hideSplash`

## Fixes aplicados

### A. CROSSFADE real splash → dashboard

**CSS:**
```css
#app {
  opacity: 0;
  transition: opacity 0.50s cubic-bezier(0.4, 0.0, 0.2, 1);
}
body[data-app-ready="true"] #app {
  opacity: 1;
}
```

**JS:** al iniciar fade-out del splash, marcar `body[data-app-ready="true"]` → fade-in simultáneo del dashboard. Resultado: crossfade real, sin "salto" del dashboard apareciendo detrás.

### B. `_hideSplash` IDEMPOTENTE estricto

**Antes:**
```js
if (!splash || splash.classList.contains('hidden')) return;
```
Permitía re-ejecutar el `setTimeout` durante `.fade-out`.

**Ahora:**
```js
if (splash.classList.contains('hidden') || splash.classList.contains('fade-out')) return;
```
Una vez iniciado el fade-out, llamadas posteriores son no-op estrictas.

### C. Safety net 8s usa función idempotente

**Antes:** safety net L41317 manipulaba `.fade-out` y `.hidden` directo, podía interferir si `_hideSplash` ya estaba en ejecución.

**Ahora:**
```js
if (window._hideSplash) window._hideSplash();
if (!document.body.getAttribute('data-app-ready')) {
  document.body.setAttribute('data-app-ready', 'true');
}
```

### D. Animación más suave

- Transition splash fade-out: `0.40s var(--ease-standard)` → `0.50s cubic-bezier(0.4, 0.0, 0.2, 1)` (Material standard ease-in-out)
- Mismo timing function en `#app` fade-in → ambos se mueven con la misma curva
- 500ms es más "premium" que 400ms sin perceptible delay

## Control de ciclo final

```
1. HTML carga → splash visible (display:flex), #app oculto (opacity:0)
2. CSS animations splash entran (icono spring 520ms + tagline cascade 700ms)
3. JS carga → load() localStorage → render() Dashboard (oculto por opacity:0)
4. Firebase init paralelo (no bloquea)
5. _hideSplash invocado:
   - Garantiza MIN_VISIBLE 1000ms desde inicio
   - Marca body[data-app-ready="true"] → #app fade-in 500ms
   - splash añade .fade-out → fade-out 500ms
   - Ambos en paralelo → crossfade real
6. Tras 420ms: splash añade .hidden → display:none
7. App lista
```
...
- ✅ 30 bloques JS OK
- ✅ Animación entra una sola vez (CSS animation con `animation-fill-mode:both`)
- ✅ Crossfade splash ↔ dashboard sincronizado (mismo easing, misma duración)
- ✅ Sin frame intermedio vacío (#app oculto hasta data-app-ready)
- ✅ `_hideSplash` idempotente estricto (sin re-entrancy)
- ✅ Dashboard como módulo inicial (modo `hub`: tab=1)
- ✅ Hidratación silenciosa: load() síncrono, Firebase async sin bloquear

## Edge cases cubiertos

| Caso | Comportamiento |
|------|----------------|
| Arranque frío | Splash 1000ms + crossfade → Dashboard con localStorage |
| Arranque offline | Splash → Dashboard (sin auth-screen, modo local) |
| Multi-device | Crossfade igual; sync remoto llega después sin tocar splash |
| Datos vacíos | Dashboard renderiza vacío durante crossfade |
| Firebase tarda >1.5s | Timeout dispara `_hideSplash` → crossfade sin auth-screen |
| Sesión perdida | auth-changed dispara post-crossfade → auth-screen entra después |

## Riesgos prevenidos

- **`auth-changed` múltiple**: `_hideSplash` idempotente estricto
- **Salto visual dashboard**: `#app` con opacity:0 hasta data-app-ready
- **Safety net interfiere**: usa `_hideSplash` en lugar de manipular clases
- **Frame en blanco**: splash sigue visible hasta que #app está fade-in
- **Easing inconsistente**: mismo cubic-bezier en splash y #app

### v1.3.1169 — RENDER TRAILING DEBOUNCE + AUDIT UI

**Auditoría sistema UI (sin tocar código):**

| Área | Estado | Acción |
|------|--------|--------|
| Tipografía | 280+ usos hardcoded `font-size:Xpx`. Sistema `--fs-*` cubre nuevas UI ✓ | NO refactor masivo (riesgo regresión) |
| Spacing | Mayoría sigue escala 4pt. Sistema `--sp-*` ya existe ✓ | NO cambios |
| Componentes | `btn-ok` (77 usos), `fi` inputs (105 usos), `mhd-*` modales, `hdr-*` headers ya unificados ✓ | NO cambios |
| Menús Ajustes | Jerarquía clara: Módulos → Preferencias → Cuenta → Sección dev colapsable ✓ | NO duplicados |
| Render perf | 21 callsites `App.dash.render()` con guard `_isRendering` que descarta concurrentes | **FIX aplicado** |

## Fix aplicado: trailing debounce en `App.dash.render()`

**Problema detectado en audit M1 v1.3.1161:** el guard `_isRendering` descarta TODOS los renders concurrentes, incluyendo el último (que puede ser el más actualizado).

**Escenario problema:**
```
t=0   evento_1 → render() → _isRendering=true (en curso 50ms)
t=10  evento_2 → render() → DESCARTADO
t=20  evento_3 → render() → DESCARTADO
t=30  evento_4 → render() → DESCARTADO  ← este tenía el estado final
t=50  render termina → _isRendering=false
t=51  ... no hay más renders, UI queda con estado de t=0 ❌
```

**Fix:**
```js
if (_isRendering) { _renderQueued = true; return; }
// ... render ...
} finally {
  _isRendering = false;
  if (_renderQueued) {
    _renderQueued = false;
    requestAnimationFrame(() => render()); // trailing
  }
}
```

**Comportamiento nuevo:**
- 1er render: ejecuta normal
- Renders 2..N concurrentes: solo activan flag `_renderQueued`
- Al terminar 1er render: si `_renderQueued`, se dispara UN solo render adicional (en rAF para evitar stack overflow)
- Resultado: UI siempre refleja el estado MÁS RECIENTE

**Beneficios:**
- Imports masivos (Loyverse 100+ ventas): 1 render inicial + 1 trailing al final, no 100 descartes
- Multi-device sync: cambios remotos no se "pierden" si llegan durante un render
- Cascadas eventos (venta → stock → cuenta): siempre converge al estado correcto

**Sin overhead:**
- Si no hay concurrencia, comportamiento idéntico a antes
- Solo 1 render trailing máximo (no encadena infinitamente)
- `requestAnimationFrame` garantiza que se ejecuta solo cuando el browser está libre

## Validación

- ✅ 30 bloques JS OK
- ✅ Sin cambios en flujo normal (1 evento → 1 render)
- ✅ Cascadas reflejan estado final
- ✅ Sin riesgo stack overflow (rAF + flag binario)

## Pendiente sesión dedicada (sistema UI completo)

Spec del usuario completo (tipografía/spacing/menús/componentes) requiere:
1. Auditoría con cuantificación (tomada en esta sesión)
2. Plan refactor por módulo con QA por cada uno
3. Eliminación gradual de hardcoded → tokens
4. Sin "big bang" — cada módulo en su release independiente

**Estimado:** 5-8 sesiones para refactor completo seguro.
